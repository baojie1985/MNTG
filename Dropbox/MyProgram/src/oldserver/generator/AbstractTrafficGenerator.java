...
public abstract class AbstractTrafficGenerator implements TrafficGenerator {
	...
	public List<File> getCounties(TrafficRequest trafficRequest) {
		...
		try {
			pstatement = conn
					.prepareStatement("select filename from counties C where C.upperlong < ? and C.lowerlong > ? and C.upperlat > ? and C.lowerlat < ?");
			...
			while (rs.next()) {
				counties.add(new File(Controller.BASE_SHAPE_FILE_PATH, rs
						.getString(1) + File.separator + rs.getString(1)));
			}
